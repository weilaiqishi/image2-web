...
import { awesomePromptsAdapter, image2NetAdapter } from "./prompt-sources/index.mjs";
...
const adapters = [image2NetAdapter, awesomePromptsAdapter];
...
async function cacheThumbnail(record) {
  ...
  if (!record.previewUrl) return undefined;
  try {
...
for (const [index, record] of merged.entries()) {
  const cachedThumbnailPath = await cacheThumbnail(record);
  if (cachedThumbnailPath) available.push({ ...record, cachedThumbnailPath });
  process.stdout.write(`\rCaching ${index + 1}/${merged.length}`);
}

const generatedAt = new Date().toISOString();
const catalogVersion = generatedAt.replace(/[-:TZ.]/g, "").slice(0, 14);
const items = available.sort((left, right) => left.sourceId.localeCompare(right.sourceId) || left.title.localeCompare(right.title));
