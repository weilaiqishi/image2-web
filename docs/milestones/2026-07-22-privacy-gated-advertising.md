...
比较范围为本地 `main`（`202f3b0`）到当前 `HEAD`（`4fc12bb`）：
...
`origin/main` 当前位于 `417c28b`，比本地 `main` 多一个提示词目录刷新提交。当前分支与 `origin/main` 已分叉，各自领先 1 个提交；本里程碑只统计 `main...HEAD` 中的广告功能，不把远端目录刷新算入，也不声称当前分支已经包含该刷新。
